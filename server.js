...
app.post('/addprojet', (req, res) => {
  const { titre, prix, description, delailiv, IDC, IDR } = req.body;

  
  if (!titre || !prix || !description || !delailiv || !IDC || !IDR) {
      return res.status(400).send('All fields are required.');
  }

  
  const insertQuery = 'INSERT INTO projet (titre, prix, description, delailiv, IDC, IDR) VALUES (?, ?, ?, ?, ?, ?)';
  connection.query(insertQuery, [titre, prix, description, delailiv, IDC, IDR], (error, results, fields) => {
      if (error) {
